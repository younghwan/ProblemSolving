...
class MySolution {
    fun solution(relation: Array<Array<String>>): Int {
        ...
        val colsize: Int = relation[0].size
        val candidates: MutableList<Int> = LinkedList()
        for (i in 1 until (1 shl colsize)) {
            if (check(relation, rowsize, colsize, i)) candidates.add(i)
        }
        candidates.sortWith(comp)
        while (candidates.size != 0) {
            val n = candidates.removeAt(0)
            answer++
    ...
    var comp: Comparator<Int> = object : Comparator<Int> {
    ...
    private fun check(relation: Array<Array<String>>, rowsize: Int, colsize: Int, subset: Int): Boolean {
